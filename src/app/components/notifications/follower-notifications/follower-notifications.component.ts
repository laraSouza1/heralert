...
@Component({
  selector: 'app-follower-notifications',
  ...
  templateUrl: './follower-notifications.component.html',
  styleUrl: './follower-notifications.component.css'
})
export class FollowerNotificationsComponent {
  ...
  ngOnInit() {
    ...
    if (user?.id) {
      ...
      this.http.get<any>(`http://localhost:8085/api/notifications/${this.userId}`).subscribe(res => {
        if (res.status) {
          this.notifications = res.data.filter((n: any) => n.type === 'follow');
